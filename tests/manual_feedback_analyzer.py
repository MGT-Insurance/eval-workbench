...
- High-risk zone for hail damage"""
                ...
                HumanMessage(
                    content='Yes, please update. The new roof is architectural shingles with 30-year warranty.'
                ),
                AIMessage(
...
- Alcohol percentage needs verification"""
                ...
                HumanMessage(
                    content="Fine. Just get it fixed. I'm done dealing with this today."
                ),
            ]
        ),
        additional_input={
...
- Fuel storage details not found"""
                ...
                HumanMessage(
                    content='Correct. They have a separate contract with the fuel dock next door.'
                ),
                AIMessage(
                    content="""**Updated Assessment**
...
- Adequate safety measures in place"""
                ...
                AIMessage(
                    content="Thank you for the approval. I'll update the case status to Approved."
                ),
...
System Error: Premium calculation returned $43.03 which appears incorrect for a $2M coverage limit."""
                ...
                HumanMessage(
                    content='Can you flag this for the dev team? This is the third time this month.'
                ),
                AIMessage(
                    content="I've logged the issue. Ticket #DEV-789 created for the engineering team."
                ),
                HumanMessage(
                    content="Thanks. I'll manually calculate the premium for now. $4,303 is correct."
                ),
            ]
        ),
        additional_input={
...
- Ski equipment rental shop"""
                ...
                HumanMessage(
                    content="Quick question - does this include the outdoor hot tub area? That's usually a separate liability consideration."
                ),
                AIMessage(
...
async def run_tests():
    ...
    frustrated_count = sum(
        1 for _, r in all_results if r.signals.sentiment.is_frustrated
    )
    resolved_count = sum(1 for _, r in all_results if r.signals.resolution.is_resolved)

    print(f'\nTotal conversations: {len(all_results)}')
    print(
        f'Interventions: {intervention_count} ({100 * intervention_count / len(all_results):.0f}%)'
    )
    print(
        f'STP (no escalation): {stp_count} ({100 * stp_count / len(all_results):.0f}%)'
    )
    print(
        f'Frustrated users: {frustrated_count} ({100 * frustrated_count / len(all_results):.0f}%)'
    )
    print(
        f'Resolved: {resolved_count} ({100 * resolved_count / len(all_results):.0f}%)'
    )
