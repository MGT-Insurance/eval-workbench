...
class FormattingIssue(RichBaseModel):
    issue_type: str = Field(..., description="The type of formatting error (e.g., 'Forbidden Double Asterisk').")
...
class SlackFormattingCompliance(BaseMetric):
    ...
    async def execute(self, item: DatasetItem, **kwargs) -> MetricEvaluationResult:
        ...
        explanation = "Formatting: Perfect." if score == 1.0 else f"Formatting Issues Found: {[i.issue_type for i in issues]}"
    ...
    def get_signals(self, result: FormattingResult) -> List[SignalDescriptor]:
        signals = []
        signals.append(SignalDescriptor(name="formatting_score", description="Score", extractor=lambda r: r.score, headline_display=True))
        for i, issue in enumerate(result.issues):
            signals.extend([
                SignalDescriptor(name="issue_type", group=f"❌ Issue {i+1}", description="Type", extractor=lambda r, idx=i: r.issues[idx].issue_type),
                SignalDescriptor(name="context", group=f"❌ Issue {i+1}", description="Context", extractor=lambda r, idx=i: r.issues[idx].context),
            ])
        return signals
