...
from typing import List, Dict, Any, Optional, Union, Set, TypedDict, Literal
...
class SlackConfig:
    ...
    @classmethod
    def get_token(cls, agent_id: Optional[str] = None, override_token: Optional[str] = None) -> str:
...
VALID_EVENT_TYPES = {
    'athena': ['referrals'],
    'quill': ['binds'],
    'canary': ['evaluations', 'openprs', 'weekly-summary', 'release-train', 'releases', 'new-users', 'new-repo', 'new-swallow-projects'],
    'aimee': ['inquiries'],
            ...
            try:
                ...
                async with session.request(
                    ...
                    try:
                        ...
                        payload = {'ok': False, 'error': f'Invalid JSON response: {text[:100]}'}
                    ...
                    if status == 429 or payload.get('error') == 'ratelimited':
                        ...
                        logger.warning("[Slack API] Rate limited (attempt %s), sleeping %.2fs", attempt, delay)
                    ...
                    if status >= 500 and attempt < self._max_attempts:
                        ...
                        logger.warning("[Slack API] Server error %s (attempt %s), sleeping %.2fs", status, attempt, delay)
                        await asyncio.sleep(delay)
                        continue

                    if not payload.get('ok'):
                        logger.error("[Slack API] Request failed: %s", payload.get('error'))
                ...
                logger.warning("[Slack API] Request error (attempt %s): %s", attempt, exc)
                await asyncio.sleep(delay)
...
class SubscriptionStorage(ABC):
    ...
    @abstractmethod
    async def get_subscriptions(self, agent_id: str, event_type: str = None, active_only: bool = True) -> List[SlackSubscription]:
        pass

    @abstractmethod
    async def deactivate_subscription(self, agent_id: str, event_type: str, channel_id: str, filters: Dict[str, str]) -> bool:
        pass
...
class InMemorySubscriptionStorage(SubscriptionStorage):
    ...
    async def get_by_user(self, agent_id: str, user_id: str) -> List[SlackSubscription]:
        return [s for s in self._subs if s.agent_id == agent_id and s.created_by_slack_user_id == user_id and s.active]
...
class SlackScraper:
    ...
    def extract_text_from_attachments(attachments: List[Dict[str, Any]]) -> List[str]:
        ...
        for attachment in attachments:
            ...
            if 'blocks' in attachment:
                texts.extend(SlackScraper.extract_text_from_blocks(attachment['blocks']))
    ...
    def simplify_message(raw: Dict[str, Any]) -> SimplifiedMessage:
        ...
        attachment_texts = SlackScraper.extract_text_from_attachments(raw.get('attachments', []))
...
class SlackService:
    def __init__(self, storage: SubscriptionStorage = None, client: "SlackHttpClient" = None):
    ...
    async def _request(self, method: str, url: str, token: str, json_data: Any = None, data: Any = None, headers: Dict = None, params: Dict = None) -> Dict[str, Any]:
    ...
    async def post_message(self, channel: str, options: PostMessageOptions, agent_id: Optional[str] = None) -> SlackResponse:
        ...
        data = await self._request('POST', 'https://slack.com/api/chat.postMessage', token, json_data=payload)

        if not data.get('ok'):
            logger.error(f"[Slack API] Failed to post message: {data.get('error')}")
            return {'success': False, 'error': data.get('error'), 'status': data.get('_status')}

        logger.info("[Slack API] Message posted to %s", data.get('channel'))
        return {'success': True, 'ts': data.get('ts'), 'channel': data.get('channel')}

    async def update_message(self, channel: str, ts: str, options: PostMessageOptions, agent_id: Optional[str] = None) -> SlackResponse:
        ...
        data = await self._request('POST', 'https://slack.com/api/chat.update', token, json_data=payload)

        if not data.get('ok'):
            return {'success': False, 'error': data.get('error'), 'status': data.get('_status')}
        return {'success': True, 'ts': data.get('ts'), 'channel': data.get('channel')}

    async def post_threaded_message(self, channel: str, thread_ts: str, options: PostMessageOptions, agent_id: Optional[str] = None) -> SlackResponse:
    ...
    async def get_thread_replies(self, channel: str, thread_ts: str, agent_id: Optional[str] = None) -> SlackResponse:
    ...
    async def get_channel_history(self, channel: str, limit: int = 100, agent_id: Optional[str] = None) -> SlackResponse:
    ...
    async def validate_thread_exists(self, channel: str, thread_ts: str, agent_id: Optional[str] = None) -> bool:
    ...
    async def open_modal(self, trigger_id: str, view: Dict, agent_id: Optional[str] = None) -> SlackResponse:
        """Opens a modal view."""
        token = SlackConfig.get_token(agent_id)
        payload = {'trigger_id': trigger_id, 'view': view}
        data = await self._request('POST', 'https://slack.com/api/views.open', token, json_data=payload)
    ...
    async def publish_app_home(self, agent_id: str, user_id: str) -> None:
        ...
        data = await self._request('POST', 'https://slack.com/api/views.publish', token, json_data=payload)
    ...
    async def download_file(self, file: SlackFile, agent_id: Optional[str] = None) -> FileDownloadResponse:
        """
                ...
                while redirect_count < max_redirects:
                    ...
                    async with session.get(
                        ...
                        if 300 <= response.status < 400:
                            ...
                            if not location:
                                return FileDownloadResponse(success=False, error='Redirect without Location')
                            current_url = location
                            redirect_count += 1
                            continue

                        if not response.ok:
                            return FileDownloadResponse(success=False, error=f"HTTP {response.status}")

                        # Check content type for HTML error pages
                        content_type = response.headers.get('Content-Type', '')
                        if 'text/html' in content_type:
                            return FileDownloadResponse(success=False, error="Received HTML instead of file (likely auth error)")
                        ...
                        if len(content) < file.size * 0.5:
                             ...
                             if '<!DOCTYPE' in preview or '<html' in preview:
                                 return FileDownloadResponse(success=False, error="Buffer appears to be HTML error")
    ...
    async def post_to_subscribed_channels(
        ...
        if not subs:
            logger.info(f"[Multi-Channel] No subscriptions match for {agent_id}/{event_type}")
    ...
    async def post_to_all_threads(self, threads: List[SlackThread], message_options: PostMessageOptions, agent_id: str) -> None:
        """Broadcasts a reply to all tracked threads."""
        if not threads:
            return

        tasks = [
            self.post_threaded_message(t.channel_id, t.thread_ts, message_options, agent_id)
            for t in threads
        ]
    ...
    async def _find_matching_subscriptions(self, agent_id: str, event_type: str, event_data: Dict[str, str]) -> List[SlackSubscription]:
        all_subs = await self.storage.get_subscriptions(agent_id, event_type, active_only=True)
        return [s for s in all_subs if self._matches_filters(s.filters, event_data)]

    def _matches_filters(self, sub_filters: Dict[str, str], event_data: Dict[str, str]) -> bool:
        if not sub_filters:
            return True
    ...
    async def _build_app_home_blocks(self, agent_id: str, user_id: str) -> List[Dict]:
        ...
        blocks.append({
            "type": "section",
            "text": {"type": "mrkdwn", "text": info['description']}
        })
        ...
        blocks.append({
            "type": "header",
            "text": {"type": "plain_text", "text": "Your Subscriptions", "emoji": True}
        })

        if not subscriptions:
             blocks.append({
                "type": "section",
                "text": {"type": "mrkdwn", "text": "You haven't created any subscriptions yet."}
            })
        else:
            sub_list = "\n".join([f"• {s.event_type} ({self._format_filters(s.filters)})" for s in subscriptions])
            blocks.append({
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"*Subscriptions you've created:*\n{sub_list}"}
            })
...
class SlackBlockBuilder:
    ...
    @staticmethod
    def format_chat_response(message: str, citations: List[Dict] = None,
                           is_learning_worthy: bool = False,
                           is_feature_request: bool = False) -> Dict[str, Any]:
        ...
        if is_learning_worthy:
            actions.append({
                ...
                "value": json.dumps({"timestamp": datetime.now().isoformat()}) # Placeholder value
            })
