...
class FalkorSettings(BaseSettings):
    """Configuration for FalkorDB (Redis Graph) backend."""
    ...
    graph_name_template: str = Field(
        default='{agent_name}_rules',
        description='Template for per-agent graph names.',
    )

    def resolve_graph_name(self, agent_name: str) -> str:
        """Fill the graph_name_template with the given agent name."""
        return self.graph_name_template.format(agent_name=agent_name)
...
@lru_cache(maxsize=1)
def get_falkor_settings() -> FalkorSettings:
    """Return a cached FalkorSettings instance."""
    return FalkorSettings()
