...
class Message(RichBaseModel):
    ...
    timestamp_utc: Optional[datetime] = Field(None, description="Parsed UTC datetime from ts", exclude=True)
    sender: str = Field(..., description="Display name of sender")
    user_id: Optional[str] = Field(None, description="Slack user ID (U...)", exclude=True)
    is_bot: bool = Field(default=False, description="True if the message is from a system bot")
    content: str = Field(..., description="The text content of the message")
    messageUrl: Optional[str] = Field(None, description="Direct URL to this specific message")
    reply_count: int = Field(default=0, description="Number of replies to this message", exclude=True)
    is_thread_reply: bool = Field(default=False, description="True if this is a reply within a thread", exclude=True)

class ConversationContext(RichBaseModel):
    """Represents a single underwriting thread/case to be analyzed."""
    thread_id: str = Field(..., description="The unique ID of the Slack thread (from data.id)")
    channel_id: Optional[str] = Field(None, description="Slack channel ID for utilization tracking", exclude=True)
    case_id: Optional[str] = Field(None, description="Extracted Case ID (e.g., MGT-BOP-...)")
    ...
    messages: List[Message] = Field(default_factory=list, description="Chronological list of messages in the thread")
    thread_created_at: Optional[datetime] = Field(None, description="When the thread was created", exclude=True)
    thread_last_activity_at: Optional[datetime] = Field(None, description="When the last message was posted", exclude=True)
    human_participants: List[str] = Field(default_factory=list, description="List of human user IDs for MAU tracking", exclude=True)
...
class AuditResult(RichBaseModel):
    ...
    summary_of_human_input: Optional[str] = Field(
        None,
        description="A 1-sentence summary of what the human(s) said or asked."
    )
...
class ConversationMetrics(RichBaseModel):
    ...
    computed_at: datetime = Field(default_factory=_utcnow, description="When metrics were computed")
    ...
    # Time metrics
    duration_seconds: float = Field(0, description="Total duration of conversation in seconds")
    first_response_time_seconds: Optional[float] = Field(None, description="Time to first human response")
    avg_response_time_seconds: Optional[float] = Field(None, description="Average response time")

    # Ping-pong
    exchange_count: int = Field(0, description="Number of bot<->human transitions")
    ping_pong_ratio: float = Field(0, description="Ratio of exchanges to total messages")

    # Resolution
    is_resolved: bool = Field(False, description="Whether the conversation reached a resolution")
    resolution_type: Optional[str] = Field(None, description="approved, declined, stalemate, or None")
    time_to_resolution_seconds: Optional[float] = Field(None, description="Time from start to resolution")

    # Sentiment
    has_frustrated_message: bool = Field(False, description="Whether any message indicated frustration")

    # Stalemate detection (72 hours = 259200 seconds)
    is_stalemate: bool = Field(False, description="True if conversation inactive for >72 hours without resolution")
...
class AggregatedMetrics(RichBaseModel):
    ...
    stp_count: int = Field(0, description="Count of straight-through processing (no intervention)")
    ...
    clarification_rate: float = Field(0, description="Rate of clarification interventions")
    ...
    frustrated_rate: float = Field(0, description="Rate of conversations with frustrated messages")
