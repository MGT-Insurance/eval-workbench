# Langfuse Prompt Management (Athena)

The Athena Langfuse integration provides a join system that bridges Neon
database records with Langfuse distributed traces, enabling rich analysis of AI
workflows through combined case and trace data.
...
Main helper class to coordinate fetching and joining case data from Neon with
...
Pattern classes define regex patterns for extracting structured data from LLM
prompts. They inherit from `PromptPatternsBase`.
...
1. **PromptPatternsBase Inheritance** - `WorkflowPromptPatterns` and
   `ChatPromptPatterns` extend base
...
4. **Pattern Resolution** - Helper converts class to instance for all wrapped
   traces
